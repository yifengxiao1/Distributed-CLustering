...
def train():
    ...
    for epoch in range(n_epochs):
        ...
        for site in range(sites):
            data = images[
                   (epoch * sites * batch_size + site * batch_size) % 450000:(
                                                                                    epoch * sites * batch_size + site * batch_size + batch_size) % 450000 \
                       if (epoch * sites * batch_size + site * batch_size + batch_size) % 450000 != 0 else 450000,
                   :] / 255  # 手动读数据
            ...
            train_data = train_data.unsqueeze(1).reshape(train_size, 1, 32, 16).to(torch.float)
...
def clust(epoch=0):
    ...
    True_labels = labels
    # test_data = torch.tensor(images[(epoch * sites * batch_size) % 450000: ((epoch + 1) * sites * batch_size) % 450000 \
    #     if ((epoch + 1) * sites * batch_size) % 450000 != 0 else 450000, :])
    # True_labels = labels[(epoch * sites * batch_size) % 450000: ((epoch + 1) * sites * batch_size) % 450000 \
    #     if ((epoch + 1) * sites * batch_size) % 450000 != 0 else 450000]
    # with torch.no_grad():  # 训练集中不需要反向传播
    #     hash_codes = model.clistering_forward(
    #         test_data.reshape(sites * batch_size, 32, 16).unsqueeze(1).to(torch.float))
    # km = KMeans(n_clusters=180).fit(np.array(hash_codes))
...
if __name__ == '__main__':
    ...
    images = np.load('测试数据/S4data.npy')
    labels = np.load('测试数据/S4label.npy')
    code_len = 64
    n_epochs = 5
    sites = 20
    ...
    # train()
    clust()
    # np.save('实验结果/S4/64bit_Loss_20.npy', Loss)

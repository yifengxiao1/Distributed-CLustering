...
class LeNet(nn.Module):
    def __init__(self, code_len):
        ...
        self.conv1 = nn.Sequential(  # input_size=(1*8*8)
            nn.Conv2d(1, 6, 5, 1, 2),  # padding=2 保证输入输出尺寸相同
            nn.ReLU(),  # input_size=(6*16*8)
            nn.MaxPool2d(kernel_size=2, stride=2), )  # output_size=(6*4*4)
        self.conv2 = nn.Sequential(nn.Conv2d(6, 16, 3),
                                   nn.ReLU(),  # input_size=(16*2*2)
                                   nn.MaxPool2d(2, 2))  # output_size=(16*1*1) )
        self.fc1 = nn.Sequential(nn.Linear(16 * 1 * 1, self.code_len), nn.Tanh())
...
def train():
    ...
    for epoch in range(n_epochs):
        ...
        for site in range(sites):
            data = images[
                   (epoch * sites * batch_size + site * batch_size) % 300000:(
                                                                                    epoch * sites * batch_size + site * batch_size + batch_size) % 300000 \
                       if (epoch * sites * batch_size + site * batch_size + batch_size) % 300000 != 0 else 350000,
                   :] / 255  # 手动读数据
            ...
            train_data = train_data.unsqueeze(1).reshape(train_size, 1, 8, 8).to(torch.float)
...
def clust(epoch=0):
    # test_data = torch.tensor(images[(epoch * sites * batch_size) % 300000: ((epoch + 1) * sites * batch_size) % 300000 \
    #     if ((epoch + 1) * sites * batch_size) % 300000 != 0 else 300000, :])
    # True_labels = labels[(epoch * sites * batch_size) % 350000: ((epoch + 1) * sites * batch_size) % 300000 \
    #     if ((epoch + 1) * sites * batch_size) % 300000 != 0 else 300000]
    # with torch.no_grad():  # 训练集中不需要反向传播
    #     hash_codes = model.clistering_forward(
    #         test_data.reshape(sites * batch_size, 8, 8).unsqueeze(1).to(torch.float))
    # km = KMeans(n_clusters=120).fit(np.array(hash_codes))
    ...
    codes = np.load('实验结果/S1/64bitcodes.npy')
    codes = list(set(codes))
    num_codes = len(codes)
    print('第', epoch + 1, '轮: Cost=', num_params * 32 * 21 * sites + (32 + code_len) * num_codes)
...
if __name__ == '__main__':
    ...
    images = np.load('测试数据/S1data.npy')
    labels = np.load('测试数据/S1label.npy')
    code_len = 64
    n_epochs = 5
    sites = 20
    ...
    # train()
    clust()
    # np.save('实验结果/S1/64bit_Loss_100.npy', Loss)
    # torch.save(model, 'S1LeNet.pth')
    # np.save('实验结果/S1/64bit_MI.npy', Mi)
    # np.save('实验结果/S1/64bit_Purity.npy', Purity)

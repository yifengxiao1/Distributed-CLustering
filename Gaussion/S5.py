...
class LeNet(nn.Module):
    def __init__(self, code_len):
        ...
        self.conv2 = nn.Sequential(nn.Conv2d(6, 16, 5),
                                   nn.ReLU(),  # input_size=(16*12*12)
                                   nn.MaxPool2d(2, 2))  # output_size=(16*6*6) )
        self.fc1 = nn.Sequential(nn.Linear(16 * 6 * 6, self.code_len), nn.Tanh())
...
def train():
    ...
    for epoch in range(n_epochs):
        ...
        for site in range(sites):
            data = images[
                   (epoch * sites * batch_size + site * batch_size) % 500000:(
                                                                                    epoch * sites * batch_size + site * batch_size + batch_size) % 500000 \
                       if (epoch * sites * batch_size + site * batch_size + batch_size) % 500000 != 0 else 500000,
                   :] / 255  # 手动读数据
            ...
            train_data = train_data.unsqueeze(1).reshape(train_size, 1, 32, 32).to(torch.float)
...
def clust(epoch):
    # test_data = torch.tensor(images[(epoch * sites * batch_size) % 500000: ((epoch + 1) * sites * batch_size) % 500000 \
    #     if ((epoch + 1) * sites * batch_size) % 500000 != 0 else 500000, :])
    # True_labels = labels[(epoch * sites * batch_size) % 500000: ((epoch + 1) * sites * batch_size) % 500000 \
    #     if ((epoch + 1) * sites * batch_size) % 500000 != 0 else 500000]
    test_data = torch.tensor(images)
    True_labels = labels
    # with torch.no_grad():  # 训练集中不需要反向传播
    #     hash_codes = model.clistering_forward(
    #         test_data.reshape(-1, 32, 32).unsqueeze(1).to(torch.float))
    # km = KMeans(n_clusters=200).fit(np.array(hash_codes))
...
if __name__ == '__main__':
    ...
    images = np.load('测试数据/S5data.npy')
    labels = np.load('测试数据/S5label.npy')
    code_len = 64
    n_epochs = 5
    sites = 20
    ...
    # train()
    clust(0)
    # np.save('实验结果/S5/64bit_Loss_100.npy', Loss)
    # torch.save(model, 'S4LeNet.pth')
    # np.save('实验结果/S4/64bit_MI.npy', Mi)
    # np.save('实验结果/S4/64bit_Purity.npy', Purity)

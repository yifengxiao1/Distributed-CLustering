...
if __name__ == '__main__':
    ...
    images = np.load('测试数据/G9data.npy')
    labels = np.load('测试数据/G9label.npy')
    code_len = 64
    n_epochs = 1500
    sites = 30
    ...
    Purity = list()
    train()
    torch.save(model, 'G9LeNet.pth')
    np.save('实验结果/G9/64bit_MI.npy', Mi)
    # np.save('实验结果/G9/64bit_Purity.npy', Purity)

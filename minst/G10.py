...
def clust(epoch):
    ...
    with torch.no_grad():  # 训练集中不需要反向传播
        hash_codes = model.clistering_forward(
            test_data.reshape(sites * batch_size, 32, 32).unsqueeze(1).to(torch.float))
...
if __name__ == '__main__':
    ...
    images = np.load('测试数据/G10data.npy')
    labels = np.load('测试数据/G10label.npy')
    code_len = 64
    n_epochs = 1000
    sites = 50
    ...
    Purity = list()
    train()
    torch.save(model, 'G10LeNet.pth')
    np.save('实验结果/G10/64bit_MI.npy', Mi)
    np.save('实验结果/G10/64bit_Purity.npy', Purity)

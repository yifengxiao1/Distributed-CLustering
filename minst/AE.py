...
class AE(nn.Module):
    def __init__(self, code_len):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Flatten(),  # b, 8 * 2 * 2
            nn.Linear(8 * 2 * 2, self.code_len),
            nn.Tanh()

        )
        self.decoder = nn.Sequential(
            nn.Linear(self.code_len, 8 * 2 * 2),
            nn.ReLU(True),
...
def clust(epoch):
    ...
    with torch.no_grad():  # 训练集中不需要反向传播
        hash_codes = model.clustering_forward(
            test_data.reshape(sites * batch_size, 28, 28).unsqueeze(1).to(torch.float))
...
if __name__ == '__main__':
    ...
    Purity = list()
    train()
    np.save('实验结果/AE/40bit_MI.npy', Mi)
    np.save('实验结果/AE/40bit_Purity.npy', Purity)

...
def train():
    ...
    for epoch in range(n_epochs):
        ...
        for site in range(sites):
            data = images[
                   (epoch * sites * batch_size + site * batch_size):(epoch * sites * batch_size + site * batch_size + batch_size),:] -0.5
            ...
            loss = torch.norm((torch.norm(data_dist, dim=1) * a - torch.norm(output_dist, p=1, dim=1)) * torch.exp(torch.norm(data_dist, dim=1)), p=1) /((batch_size-1)*batch_size/2)
        ...
        torch.save(model, 'satellite/model/'+'{a}'.format(a=epoch + 1)+'.pth')
        clust(epoch)
...
if __name__ == '__main__':
    ...
    Purity = list()
    train()
    torch.save(model, 'satellite/6bit_linear.pth')
    np.save('satellite/6bit_MI.npy', Mi)
    np.save('satellite/6bit_Purity.npy', Purity)
